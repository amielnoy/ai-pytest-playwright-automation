"""
Session 6 conftest — what's NEW vs session 5
─────────────────────────────────────────────
Session 5 had:  heal_tracker, screenshot_on_failure
Session 6 adds: browser_agent  ← wraps run_agent() as a callable fixture
                The agent can drive the browser with plain English goals.

Run:  pytest course/session_6_mcp_agents/ -v
...
# ── carried forward from session 5 ────────────────────────────────────────────
...
# ── NEW in session 6 — AI agent ───────────────────────────────────────────────

@pytest.fixture
def browser_agent(page: Page):
    """NEW in session 6 — call agent(goal) to let Claude drive the browser.

"""
Session 10 conftest — what's NEW vs session 6
─────────────────────────────────────────────
Session 6 had:  login_page, inventory_page, cart_page
Session 10 adds: heal_tracker          ← self-healing selector wrapper
                screenshot_on_failure ← auto-attach PNG on failure (autouse)

Run:  pytest course/session_10_ai_tools/ -v
...
def screenshot_on_failure(request, page: Page):
    """NEW in session 10 — save a PNG whenever a test fails."""
    yield
    node = request.node

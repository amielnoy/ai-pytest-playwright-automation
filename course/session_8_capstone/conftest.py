"""
Session 8 conftest — what's NEW vs session 7
─────────────────────────────────────────────
Session 7 had:  failure_record, CI markers
Session 8 adds: allure_screenshot  ← attaches PNG to Allure report on failure
                                     (replaces the plain file screenshot)
                pre_commit check   ← warns if ruff/mypy not configured

This is the production-grade master conftest: every fixture from every
previous session is present and stable. No more additions after this point —
only session 9's Allure environment setup is added on top.

Run:  pytest course/session_8_capstone/ -v --alluredir=allure-results
"""
import json
import os
import pathlib
...
# ── NEW in session 8 — Allure screenshot attachment ───────────────────────────

@pytest.fixture(autouse=True)
def allure_screenshot(request, page: Page):
    """NEW in session 8 — attach the failure screenshot directly into Allure."""
    yield
    node = request.node
    if getattr(getattr(node, "rep_call", None), "failed", False):
        allure.attach(

"""
Session 7 conftest — what's NEW vs session 6
─────────────────────────────────────────────
Session 6 had:  browser_agent
Session 7 adds: failure_record  ← autouse fixture that writes failed test
                                   data to failures.jsonl for CI analysis.
                pytest_configure ← registers custom markers.

Run:  pytest course/session_7_ci_cd_ai_analysis/ -v
"""
import json
import os
import pathlib
...
# ── NEW in session 7 — CI failure tracking ────────────────────────────────────

@pytest.fixture(autouse=True)
def failure_record(request):
    """NEW in session 7 — write failed test metadata to failures.jsonl in CI."""
    yield
    node = request.node

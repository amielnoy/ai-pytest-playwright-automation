"""
Session 19 conftest — final capstone fixture set
─────────────────────────────────────────────
Session 13 added reporting fixtures and Allure environment setup.
Session 19 uses the complete fixture set for the final capstone.

This is the production-grade master conftest: every fixture from every
previous session is present and stable.

Run:  pytest course/session_19_capstone/ -v --alluredir=allure-results
"""
import json
import os
import pathlib
...
# ── carried forward from session 12 ────────────────────────────────────────────
...
# ── NEW in session 19 — Allure screenshot attachment ───────────────────────────

@pytest.fixture(autouse=True)
def allure_screenshot(request, page: Page):
    """NEW in session 19 — attach the failure screenshot directly into Allure."""
    yield
    node = request.node
    if getattr(getattr(node, "rep_call", None), "failed", False):
        allure.attach(

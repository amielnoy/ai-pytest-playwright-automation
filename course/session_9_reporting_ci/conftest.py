"""
Session 9 conftest — what's NEW vs session 8
─────────────────────────────────────────────
Session 8 had:  allure_screenshot (attach PNG to Allure)
Session 9 adds: pytest_configure writes categories.json + environment.properties
                into allure-results/ BEFORE any test runs, so the generated
                Allure 3 report shows categories and build context on the
                Overview tab.

This is the FINAL conftest — the fully evolved framework fixture set.

Run:  pytest course/session_9_reporting_ci/ -v --alluredir=allure-results
...
# ── NEW in session 9 — Allure categories + environment ────────────────────────
...
# ── carried forward from session 8 ────────────────────────────────────────────

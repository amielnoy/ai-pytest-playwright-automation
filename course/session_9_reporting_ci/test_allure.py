"""
Session 9 — Allure 3 reporting: the final, fully-decorated test suite.

Every test has:
  @allure.feature / @allure.story / @allure.title / @allure.severity
  with allure.step(...)  wraps each logical action

The conftest.py (session 9) also writes categories.json + environment.properties
...
def test_validate_decorators_on_this_file(tmp_path):
    """The session 9 test file itself should pass decorator validation."""
    from pathlib import Path

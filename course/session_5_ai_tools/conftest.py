"""
Session 5 conftest — what's NEW vs session 4
─────────────────────────────────────────────
Session 4 had:  login_page, inventory_page, cart_page
Session 5 adds: heal_tracker          ← self-healing selector wrapper
                screenshot_on_failure ← auto-attach PNG on failure (autouse)

Run:  pytest course/session_5_ai_tools/ -v
...
def screenshot_on_failure(request, page: Page):
    """NEW in session 5 — save a PNG whenever a test fails."""
    yield
    node = request.node

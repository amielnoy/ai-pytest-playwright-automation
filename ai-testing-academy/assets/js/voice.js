...
let onFinalCb = null;
let onStateCb = null;
function clearTimer() { if (timer !== null) {
    clearInterval(timer);
    timer = null;
} }
function deliver() {
...
function finish() {
    ...
    try {
        ...
        deliver();
    } // rec.onend delivers; if stop() throws, deliver now
}
function tick() {
    const now = Date.now();
    ...
    if (elapsed >= MIN_MS && silence >= MAX_PAUSE_MS)
        finish();
}
...
export function toggleRecognition(onFinal, onState) {
    ...
    if (rec) {
        finish();
        return;
    } // a second tap stops and submits
    ...
    rec.onend = () => {
        if (finalizing || Date.now() - startedAt >= MAX_MS) {
            deliver();
            return;
        }
        try {
            rec.start();
        } // browser ended early — keep listening
        catch {
            setTimeout(() => { if (rec && !finalizing) {
                try {
                    rec.start();
                }
                catch { /* give up */ }

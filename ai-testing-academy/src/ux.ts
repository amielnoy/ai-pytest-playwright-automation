...
function initVideos(): void {
  document.querySelectorAll('.video-card').forEach(card => {
    ...
    ph.className = 'no-video'; ph.textContent = S.videoLoading;
...
function initScrollSpy(): void {
  const links = ([...document.querySelectorAll('nav a.link')] as HTMLAnchorElement[])
    .filter(a => (a.getAttribute('href') || '').startsWith('#'));
  const byId: Record<string, HTMLAnchorElement> =
    Object.fromEntries(links.map(a => [(a.getAttribute('href') || '').slice(1), a]));
...
function initReveal(): void {
  const targets = document.querySelectorAll(
    'section > h2, section > .lead, section .card, section .agent-box, section pre, section table, section h3, section ul, section p');
...
function initChrome(): void {
  ...
  document.querySelectorAll("nav a.link[href^='#']").forEach(a => {
    ...
    if (num) {
      ...
      s.className = 'nav-num'; s.setAttribute('aria-hidden', 'true'); s.textContent = num;
      a.prepend(s);
    }
  });
